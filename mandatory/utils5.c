...
/*   Updated: 2024/01/29 13:10:57 by eates            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (x->map.map[(int)((x->py[1] + 2 * a[1] - 1) / PIXEL)]
		[(int)(x->px[1] / PIXEL)] == '1')
		return (x->which_wall = 2 * a[1], x->player.new_x = x->px[1],
			x->player.new_y = x->py[1], 1);
	if (x->map.map[(int)((x->py[1] + 2 * a[1] - 1) / PIXEL)]
		[(int)((x->px[1] + 2 * a[0] - 1) / PIXEL)] == '1')
	{
		if ((!a[0] && !a[1]) || x->map.map[(int)(x->py[1] / PIXEL)]
			[(int)((x->px[1] + 2 * a[0] - 1) / PIXEL)] == '1')
			return (x->which_wall = 3, x->player.new_x = x->px[1],
				x->player.new_y = x->py[1], 1);
		return (x->which_wall = 2 * a[1], x->player.new_x = x->px[1],
			x->player.new_y = x->py[1], 1);
	}

...
static void control2(t_so_long *s)
{
	int x;
	int y;
...
int handle_key(int keycode, t_so_long *s)
{
	...
	render(s, -1);
	if (keycode == 13)
		mlx_put_image_to_window(s->ptr, s->win, s->back, s->p.x, s->p.y);
	else if (keycode == 0)
		mlx_put_image_to_window(s->ptr, s->win, s->left, s->p.x, s->p.y);
	else if (keycode == 1)
		mlx_put_image_to_window(s->ptr, s->win, s->front, s->p.x, s->p.y);
	else if (keycode == 2)
		mlx_put_image_to_window(s->ptr, s->win, s->right, s->p.x, s->p.y);
	return (0);
}

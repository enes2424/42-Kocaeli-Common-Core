...
static inline void print_extra(t_so_long *s)
{
	char *str_score;
	char *itoa_score;
	char *str_num_of_steps;
	char *itoa_num_of_steps;

	itoa_score = ft_itoa(s->p.score);
	itoa_num_of_steps = ft_itoa(s->num_of_steps);
	str_score = ft_strjoin("Score : ", itoa_score);
	str_num_of_steps = ft_strjoin(itoa_num_of_steps, " steps moved");
	mlx_put_image_to_window(s->ptr, s->win,
							s->creeper[s->index_creeper / 15], s->creeper_x, s->creeper_y);
	mlx_put_image_to_window(s->ptr, s->win, s->tmp_player, s->p.x, s->p.y);
	mlx_string_put(s->ptr, s->win, UNIT / 2 - 2, UNIT / 2 - 10, 0, str_score);
	mlx_string_put(s->ptr, s->win, UNIT / 2 - 2,
				   UNIT / 2 + 10, 0, str_num_of_steps);
	free(itoa_score);
	free(str_score);
	free(itoa_num_of_steps);
	free(str_num_of_steps);
}
